...
| **A4** | [`plan/phase_A4_lab.md`](./plan/phase_A4_lab.md) | Lab pipeline: receive → samples → tests → result entry → supervisor review → certificates → rejection → documents | ~7 | A1 | ✅ completed |
| **A5** | [`plan/phase_A5_manufacturer.md`](./plan/phase_A5_manufacturer.md) | Manufacturer chain: marketplace → procurement → GRN → inventory → production → products → product QR → lineage → recall impact | ~10 | A1, A2 (shipments) | ✅ completed |
